...
import { TRIP_REPOSITORY, type TripRepository } from '../domain/repositories/trip.repository.interface';
...
export class TripsService {
    ...
    async search(searchTripDto: SearchTripDto): Promise<Trip[]> {
        ...
        try {
            ...
            this.logger.error(`Unexpected search error: ${error.message}`, error.stack);
            throw new UnprocessableEntityException('An unexpected error occurred while searching for trips');
        }
    }
}
